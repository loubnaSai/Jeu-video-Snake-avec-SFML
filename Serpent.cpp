 ...
 void serpent::changer_fruit_pos(sf::RenderWindow &window){
    ...
    int randomY ;
      ...
      do{
        ...
        for(int h=0;h<k;h++)    if(randomX*32==Body[i].get_pos().x /*|| randomX*32==Body[i].get_pos().x+32 || randomX%32!=0*/ || randomX*32==bord[h].get_pos().x /*|| randomX==bord[h].get_pos().x+32*/ ) j++;
        }
      }while(j>0);
      do{
             j=0;
        ...
        for(int i=0;i<longeur;i++){
        for(int h=0;h<k;h++)    if(randomY*32==Body[i].get_pos().y || /*randomY==Body[i].get_pos().y+32|| randomY%32!=0 || */randomY*32==bord[h].get_pos().y /*|| randomY==bord[h].get_pos().y +32*/) j++;
        }
      }while(j>0);
...
void serpent::move_body(){
   // Body[0].setPos(mSnakes[0].getPosition().x,mSnakes[0].getPosition().y);
...
if (longeur> 1)
        {
            ...
            {
                //mSnakes[i].setPosition(Body[i-1].get_pos().x, Body[i-1].get_pos().y);
               // Body[i].setPos(Body[i-1].get_pos().x, Body[i-1].get_pos().y);
...
void serpent::is_injured_itself(sf::RenderWindow &window){
     for (int i = 2; i < longeur; i++) // Snake's boxes
    {
...
void serpent::move(sf::RenderWindow &window){
  ...
  is_colliding_wall(window);
   if (Body[0].get_pos().x == fruit.get_pos().x // Food collision
